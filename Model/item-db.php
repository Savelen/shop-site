...
function get_item($start, $gender, $get = 3, $kids = 0){
	global $pdo;
	if ($kids != '*') {
		$sql = "SELECT * FROM item WHERE gender = :gender AND kids = :kids LIMIT :start,:get";
	}
	else {
		$sql = "SELECT * FROM item WHERE gender = :gender LIMIT :start,:get";
	}
	$pre = $pdo->prepare($sql);
	...
	$return = prepare_img($return);
	return $return;
}

function get_img($id){
	global $pdo;
	$sql = "SELECT * FROM item_img WHERE id_img = :id";
	$pre = $pdo->prepare($sql);
	$pre->execute([":id" => $id]);
	return $pre->fetchAll(PDO::FETCH_OBJ);
}
...
function get_item_id($id){
	...
	$pre->execute();
	return prepare_img($pre->fetchAll(PDO::FETCH_OBJ));
}
function prepare_img($data){
	foreach ($data as $item) {
		$buf = get_img($item->id);
		$item->img = [];
		$item->color = [];
		for ($i = 0; $i < count($buf); $i++) {
			$buf[$i]->url_img = explode(",", $buf[$i]->url_img);
			array_push($item->img, $buf[$i]->url_img);
			array_push($item->color, $buf[$i]->color_img);
		}
	}
	return $data;
}
...
function sql_color($name){
	global $pdo;
	$sql = "SELECT * FROM `color_value` WHERE name = :name";
	$pre = $pdo->prepare($sql);
...
function color($name){
	...
	if ($color[0]->rgb == null || $color[0]->rgb != $hec) {
		global $pdo;
		$sql = "UPDATE `color_value` SET rgb = :hec WHERE name = :name";
		$pre = $pdo->prepare($sql);
...
function add_color($name, $red = 0, $green = 0, $blue = 0, $rgb = null){
	global $pdo;
	$sql = 'INSERT INTO `color_value`(name,red,green,blue,rgb) VALUES (:name,:red,:green,:blue,:rgb)';
	$pre = $pdo->prepare($sql);
	$pre->execute(["name" => $name, "red" => $red, "green" => $green, "blue" => $blue, "rgb" => $rgb]);
}
// print_r(get_item(0,'men'));
// echo "<br>----------------------------------------------------------------<br>";
// print_r(get_img(1));
// echo "<br>----------------------------------------------------------------<br>";
// print_r(get_img(12));
